...
data {
  ...
  int<lower=0> N_TAF;         // Number of observations on TAF
  int<lower=0> gp_input_dim; // input dimension for GP
  vector[N_TAF] x_TAF;            // duration on TAF
  vector[N] x_bl;            // baseline BMD zscore
  array[N] int TAF;        // Indicator for TAF
  array[N] int coords_TAF;
...
parameters {
  ...
  real beta_bl;           // Coefficient for baseline zscore
...
model {
  ...
  beta_bl ~ normal(0, 1); // baseline BMD zscore
  beta_TAF ~ normal(0, 1); //TAF coeff
  sigma ~ normal(0, 1); //noise
  beta_f ~ normal(0, 1);
  //lscale ~ inv_gamma(5, 5); //lengthscale GP
  lscale ~ normal(0, 0.2);
  alpha ~ normal(0,0.5); //marginal variance/magnitude GP
  //alpha ~ normal(0, 4);
  //lscale ~ normal(0, 2); //lengthscale GP
  //alpha ~ normal(0,0.15); //marginal variance/magnitude GP
...
generated quantities {
  ...
  for (n in 1:N) {
    if(TAF[n]==0){
      y_pred[n] = normal_rng(beta0 + beta_bl * x_bl[n], sigma);
    }else{
      y_pred[n] = normal_rng(beta0 + beta_bl * x_bl[n] + beta_TAF + f_TAF[coords_TAF[n]], sigma);
    }
  }
}
